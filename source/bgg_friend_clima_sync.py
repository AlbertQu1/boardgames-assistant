"""
Clima diario para las partidas de amigos en boardgames_bgg.plays_amigos, en dos capas:

1. sync_por_ubicacion(): clima exacto por lugar, para las ubicaciones de
   boardgames_bgg.ubicaciones_amigos_alias que ya tienen lat/lon (heredadas de
   boardgames_stats.lugares cuando el nombre coincide con un lugar que Alberto tambien
   ...
   boardgames_bgg.clima_ubicacion_diario.
2. sync_proxy_generico(): clima generico de CDMX (mismas coords de "Casa" que
   usa el resto del proyecto, corregidas 2026-08-13 via Plus Code) como
   respaldo para las ubicaciones sin lat/lon todavia (casas de amigos sin
   geolocalizar, ej. "Casa Gus"). Guarda en boardgames_bgg.clima_cdmx_diario.

duracion_model.py usa el clima por ubicacion cuando existe y cae al proxy
generico si no. Ninguna de las dos tablas se fusiona con boardgames_stats.clima_diario
...
LAT, LON = 19.4371875, -99.1509375  # mismas coords que Casa (reusadas en todo el proyecto)
...
def sync_por_ubicacion() -> dict:
    ...
    cur.execute(
        ...
        FROM boardgames_bgg.ubicaciones_amigos_alias
    ...
    for ubicacion_normalizada, lat, lon in ubicaciones:
        cur.execute(
            """
            SELECT MIN(fecha), MAX(fecha) FROM boardgames_bgg.plays_amigos
        ...
        for fecha, temp, precip in zip(data["time"], data["temperature_2m_mean"], data["precipitation_sum"]):
            cur.execute(
                """
                INSERT INTO boardgames_bgg.clima_ubicacion_diario (ubicacion_normalizada, fecha, temp_media_c, precipitacion_mm)
                VALUES (%s, %s, %s, %s)
...
def sync_proxy_generico() -> dict:
    conn = psycopg2.connect(os.environ["DATABASE_URL"])
    cur = conn.cursor()
    cur.execute("SELECT MIN(fecha), MAX(fecha) FROM boardgames_bgg.plays_amigos WHERE fecha IS NOT NULL")
    fecha_min, fecha_max = cur.fetchone()
    if fecha_min is None:
        cur.close()
        conn.close()
    ...
    for fecha, temp, precip in zip(data["time"], data["temperature_2m_mean"], data["precipitation_sum"]):
        cur.execute(
            """
            INSERT INTO boardgames_bgg.clima_cdmx_diario (fecha, temp_media_c, precipitacion_mm)
            VALUES (%s, %s, %s)
